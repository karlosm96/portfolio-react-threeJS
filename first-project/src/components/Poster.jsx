...
export default function Poster(...props){
    ...
    const sequenceLength = useMemo(() =>{ return val(preliminarSheet.sequence.pointer.length); })

    // Set in the current poster timeline the starting position
    // (Calculate the start position: offset wheel * main sheet lenght)
    useFrame(()=>{
        timeLine.current.seek(scrollControl.offset * sequenceLength);
    })

    // Preload the poster animation
    useLayoutEffect(()=>{
    ...
    return(
        <a.group {...props}>
                <mesh ref={posterRef} name="posterlWelcome" geometry={nodes.cartelwelcome.geometry} material={materials.wood_cartel} position={[-0.765, -0.025, 2.795]} rotation={[0, 0.9, 0]} scale={[0.04, 0.489, 0.049]} />
        </a.group>
    )
}